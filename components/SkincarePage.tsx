...
const SkincarePage: React.FC<{
    ...
    const pageProducts = useMemo(() => {
        const baseProducts = allProducts.filter(p => p.category === 'skincare');
        let filteredProducts: Product[];

        switch (activeSubCategory) {
            case 'Limpiadores':
                filteredProducts = baseProducts.filter(p => p.name.toLowerCase().includes('limpiador') || p.name.toLowerCase().includes('espuma') || p.name.toLowerCase().includes('micelar'));
                break;
            case 'Sérums y Tratamientos':
                filteredProducts = baseProducts.filter(p => p.name.toLowerCase().includes('sérum') || p.name.toLowerCase().includes('serum') || p.name.toLowerCase().includes('niacinamida') || p.name.toLowerCase().includes('potenciador'));
                break;
            case 'Hidratantes':
                filteredProducts = baseProducts.filter(p => p.name.toLowerCase().includes('día') || (p.name.toLowerCase().includes('crema') && !p.name.toLowerCase().includes('noche')));
                break;
            case 'Tratamientos de Noche':
                filteredProducts = baseProducts.filter(p => p.name.toLowerCase().includes('noche'));
                break;
            case 'Cuidado Específico':
                 filteredProducts = baseProducts.filter(p => p.name.toLowerCase().includes('universal'));
                 break;
            case 'Todos':
            default:
    ...
    return (
        <div className="bg-white">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
                <div className="mb-4">
                    <h1 className="text-3xl font-bold text-brand-primary">Cuidado Facial</h1>
                </div>
